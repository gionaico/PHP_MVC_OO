...
function upload_files() {
    ...
    ////////////////////////////////////////////////////////////////////////////
    $upfile = $_SERVER['DOCUMENT_ROOT'].'/Programacion/ProyectoGio/media/'.$_FILES['file']['name'];
    if (is_uploaded_file($_FILES['file']['tmp_name'])){
        if (is_file($_FILES['file']['tmp_name'])) {
            ...
            $cont=0;
            $array_photos =array()
            $_SESSION['file_saved']=$nombreFichero;
            $copiarFichero = true;
            // I use absolute route to move_uploaded_file because this happens when i run ajax
            $upfile = $_SERVER['DOCUMENT_ROOT'].'/Programacion/ProyectoGio/media/'.$nombreFichero;
        }else{
                $error .=   "Invalid File...";
        }
    } 
    ...
    if ($error == "") {
        ...
        if($_FILES['file']['error'] !== 0) { //Assignarem a l'us default-avatar
            $upfile = '/Programacion/ProyectoGio/media/default-avatar.png';
            return $return=array('resultado'=>true,'error'=>$error,'datos'=>$upfile);
        }
    }else{
        return $return=array('resultado'=>false,'error'=>$error,'datos'=>"");
...
function remove_files(){
	$name = $_POST["filename"];
	if(file_exists($_SERVER['DOCUMENT_ROOT'].'/Programacion/ProyectoGio/media/'.$_SESSION['file_saved'])){
		unlink($_SERVER['DOCUMENT_ROOT'].'/Programacion/ProyectoGio/media/'.$_SESSION['file_saved']);
		return true;
	}else{
		return false;
	}
