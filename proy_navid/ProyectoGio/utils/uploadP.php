...
function upload_filesP() {
    ...
    ////////////////////////////////////////////////////////////////////////////
    $upfile = $_SERVER['DOCUMENT_ROOT'].'/proy_navid/ProyectoGio/media/products/'.$_FILES['file']['name'];
    if (is_uploaded_file($_FILES['file']['tmp_name'])){
        if (is_file($_FILES['file']['tmp_name'])) {
            ...
            $upfile = $_SERVER['DOCUMENT_ROOT'].'/proy_navid/ProyectoGio/media/products/'.$nombreFichero;
        }else{
                $error .=   "Invalid File...";
        }
    } 
    ...
    if ($error == "") {
        ...
        if($_FILES['file']['error'] !== 0) { //Assignarem a l'us default-avatar
            $upfile = '/proy_navid/ProyectoGio/media/products/default-potho.jpg';
            return $return=array('resultado'=>true,'error'=>$error,'datos'=>$upfile);
        }
    }else{
        return $return=array('resultado'=>false,'error'=>$error,'datos'=>"");
...
function remove_file(){
	$name = $_POST["filename"];
	if(file_exists($_SERVER['DOCUMENT_ROOT'].'/proy_navid/ProyectoGio/media/products/'.$_SESSION['m_newfile'])){
		unlink($_SERVER['DOCUMENT_ROOT'].'/proy_navid/ProyectoGio/media/products/'.$_SESSION['m_newfile']);
		return true;
	}else{
		return false;
	}
